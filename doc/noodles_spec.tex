...
\begin{table}
	\begin{tabularx}{.9\textwidth}{p{2.5in}X}
		...
		\midrule
		\lstinline[language=c++]| void tbl_subscribe() |
		&
		Subscribe to changes in the table. The client will then receive signals, starting with the \texttt{table\_init} signal.
		\\
		\lstinline[language=c++]| void tbl_insert_row(Row) |
		&
		Request to add data to the table.
		\\
		\lstinline[language=c++]| void tbl_insert_many([Column]) |
		&
		Request to add many rows of data to the table, as a pack of column segments.
		\\
  		\lstinline[language=c++]| void tbl_update(Key, Row) |
        &
        Request to update data to the table.
        \\
        \lstinline[language=c++]| void tbl_update_n([Key], [Column]) |
        &
...
\begin{table}
	\begin{tabularx}{.9\textwidth}{p{2.5in}X}
		...
		\midrule
		\lstinline[language=c++]| void tbl_init(/*snip*/) |
		&
