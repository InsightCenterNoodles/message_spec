...
Each instance is represented by a mat4, in column major format. Instances have a position `p`, rotation `r` (as a quaternion), texture `t`, scale `s`. Transforms should be applied in the following order: scaling, rotation, position. Texture transforms should be applied as texture scale, then texture delta.
